async function loadNewsDetail() {
    ...
    if (!id) {
        container.innerHTML = `<div class="project-detail-error">News ID is missing.</div>`;
        return;
    }

    try {
        const response = await fetch(`api/get_news.php?id=${encodeURIComponent(id)}`);
        const data = await response.json();

        if (!data || !data.id) {
            container.innerHTML = `<div class="project-detail-error">News not found.</div>`;
            return;
        }
        ...
        container.innerHTML = `<div class="project-detail-error">Unable to load news detail.</div>`;
    }
}
